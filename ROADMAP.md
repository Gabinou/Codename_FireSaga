

# Roadmap

## Design considerations
	
More details on my thoughts are available on the [development blog](https://averagebear.game.blog/):
- Dictatorial veto:
	1. Permadeath (No exceptions)
	2. No Phoenix/Casual mode
	3. No Ambush spawns
	4. NO DRAGONS

Final decision, no discussion. 
If you want a game with these features go buy FE Fates or make your own. 

- Linear, chapter-based
	- Army movement on the world map only if it makes narrative sense
	- Certain story Events unlockable on world-map
	- Side stories
	- Enemy encounters on world Map are rare
- Items
	- Breakable
	- Becomes unusable "broken trash" item when broken
	- Weapon's remaining number of uses can be increased using whetstones
- Units have two hands
	- Swords & shields
	- Dual Wielding
	- Handedness/Ambidextry
	- Carry stuff in your hands to free up inventory space
	- Cavaliers need one hand on bridle
- Questionable features:
	- IronMan Mode

	Automatic game save + deletion of previous save after every move? After every chapter? Game Over deletes your savefile?

	- Forcible IronMan Mode

	Force player into IronMan mode. The game akwats daves, no redis or retries. 

## Short-term

In no particular order:

- Turn system
	1. AI for enemy attacks
	2. Switching between armies (0.1)
- Design
	1. Weapons (1.0)
		- Types (1.0)
	2. Tiles (0.1)
	3. Maps
		- Test chapter (0.1)
		- Other chapters
	4. Units
		- Names (0.1)
		- Bases (0.1)
		- Caps (0.1)
		- Promotions (0.1)
- Animations
	1. Cursor idle (1.0) 
	2. Cursor slide (1.0) 
	3. Unit walking
	4. Unit idle
	5. Unit portraits
	6. Turn transition
- Menus
	1. Dynamic Unit Actions (0.1)
	2. Unit Statistics
	3. Unit Equipment
	4. Unit Attack
	5. Options
- Grid
	1. Unit movement on grid (1.0) 
	2. Attackable tiles (1.0)
	3. Moveable tiles (1.0)
	4. Danger tiles (1.0)
	5. Camera Pan and Zoom
- Controllers
	1. Gamepad Controller (1.0: D-Pad + Joystics + 2 buttons)
	2. Keyboard Controller (1.0: Move Keys + 2 buttons)
	3. Mouse Controller (1.0: Movement + 2 buttons)
	4. Touch Controller
	5. Contextual inputs (0.1)
	6. Mouse clickable buttons (0.1)
- Sound
	1. Sound system
	2. Sounds
	3. Songs
- Art
	1. Art style
	2. Unit concepts
	3. Unit portraits
	4. Map units
	5. Tiles (0.0.1)
	6. Custom Mouse (1.0)
	7. Title Screen
	8. World Map
- Gameplay systems
	1. Convoy (1.0)
	2. Camp (1.0)
	3. Combat (0.1)
- Story
	1. Main thrust (0.1)
	2. Chapter introductions
	3. Side-chapters
	4. Camp Events
	5. Relationship Events

## Long-term
	
- Playable on PC (Windows/Mac/Linux) 
- Playable on smartphones (iOS/Android)
- Playable on consoles?

## Credits
:copyright: Average Bear Games, made by Gabriel Taillon