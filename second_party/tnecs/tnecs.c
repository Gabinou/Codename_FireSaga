...
bool tnecs_world_breath_systems(struct tnecs_World *world) {
    ...
    world->systems_torun            = calloc(world->len_systems_torun,  sizeof(system_ptr));
...
void tnecs_custom_system_run(struct tnecs_World * world, system_ptr custom_system,
...
void tnecs_system_run_dt(struct tnecs_World * world, size_t in_system_id,
    ...
    system_ptr system          = world->systems_byphase[phase_id][sorder];
...
size_t tnecs_register_system(struct tnecs_World * world, const char * name,
    ...
    if (world->num_systems_byphase[phase] >= world->len_systems_byphase[phase]) {
        ...
        system_ptr *systems =          world->systems_byphase[phase];
...
void *tnecs_entity_get_component(struct tnecs_World *world, tnecs_entity eID,
    ...
    tnecs_component entity_typeflag =  tnecs_entityYPEFLAG(world, eID);
    void *out = NULL;
...
int tnecs_component_hash2id(struct tnecs_World *world, tnecs_hash hash) {
    TNECS_DEBUG_PRINTF("%s\n", __func__);
    int out = -1;
    for (size_t i = 0; i < world->num_components; i++) {
