...
void Game_Combat_Outcome(struct Game *sota) {
    ...
    defendant   = IES_GET_COMPONENT(sota->ecs.world, sota->combat.defendant, Unit);
    aggressor   = IES_GET_COMPONENT(sota->ecs.world, sota->combat.aggressor, Unit);
    struct Position *agg_pos_cmp, *dft_pos_cmp;
    dft_pos_cmp = IES_GET_COMPONENT(sota->ecs.world, sota->combat.defendant, Position);
    agg_pos_cmp = IES_GET_COMPONENT(sota->ecs.world, sota->combat.aggressor, Position);
