...
void Map_initUnitmap(struct Map * in_map) {
    if (in_map->unitmap == NULL) {
        arrfree(in_map->unitmap);
    }
    arrsetlen(in_map->unitmap, in_map->row_len * in_map->col_len);
...
void Map_Unit_Remove_fromPos(struct Map * in_map, const uint8_t col, const uint8_t row) {
    ...
    ecs_entity_t ontile_ent = in_map->tilemap[row * in_map->col_len + col];
    in_map->tilemap[row * in_map->col_len + col] = 0;
...
void Map_Unit_Remove_fromEntity(struct Map * in_map, ecs_world_t * in_world, const ecs_entity_t in_entity) {
    ...
    if (temp_position->onTilemap) {
        ontile_ent = in_map->tilemap[temp_position->tilemap_pos.y * in_map->col_len + temp_position->tilemap_pos.x];
    }
    SDL_assert(ontile_ent != 0);
    if (ontile_ent == in_entity) {
        in_map->tilemap[temp_position->tilemap_pos.y * in_map->col_len + temp_position->tilemap_pos.x] = 0;
    }
