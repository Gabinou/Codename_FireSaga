...
void Scroll_Text(tnecs_In *input) {
    ...
    struct Timer     *ut_arr = TNECS_C_ARRAY(input, Timer_ID);
    struct PixelFont *pf_arr = TNECS_C_ARRAY(input, PixelFont_ID);

    for (size_t order = 0; order < input->num_Es; order++) {
...
void Draw_Sprite(tnecs_In *input) {
    ...
    size_t         archetype_id = input->E_A_id;
    ...
    for (size_t order = 0; order < input->num_Es; order++) {
        ...
        tnecs_E entity     = gl_world->byA.Es[archetype_id][order];
...
void Draw_Map_Boss_Icon(tnecs_In *input) {
    ...
    /* -- Get components arrays -- */
    struct Boss     *boss_arr       = TNECS_C_ARRAY(input, Boss_ID);
    struct Sprite   *sprite_arr     = TNECS_C_ARRAY(input, Sprite_ID);
    struct Position *position_arr   = TNECS_C_ARRAY(input, Position_ID);
...
void Draw_Map_HPBar(tnecs_In *input) {
    ...
    /* -- Get components arrays -- */
    struct Position *pos_arr        = TNECS_C_ARRAY(input, Position_ID);
    struct MapHPBar *map_hp_bar_arr = TNECS_C_ARRAY(input, MapHPBar_ID);
...
void Draw_PopUp(tnecs_In *input) {
    ...
    /* -- Get components arrays -- */
    struct PopUp    *popup_arr    = TNECS_C_ARRAY(input, PopUp_ID);
    struct Position *position_arr = TNECS_C_ARRAY(input, Position_ID);
...
void Draw_Menu(tnecs_In *input) {
    ...
    /* -- Get components arrays -- */
    struct Menu *mc_arr = TNECS_C_ARRAY(input, Menu_ID);
...
void Draw_Text(tnecs_In *input) {
    ...
    /* -- Get components arrays -- */
    struct Text     *text_arr        = TNECS_C_ARRAY(input, Text_ID);
    struct Position *position_arr    = TNECS_C_ARRAY(input, Position_ID);
    SDL_assert(position_arr != NULL);
...
void Animate_Combat_onMap(tnecs_In *input) {
    ...
    /* -- Get components arrays -- */
    struct CombatAnimation  *combatanim_arr = TNECS_C_ARRAY(input, CombatAnimation_ID);
    struct Timer            *timer_arr      = TNECS_C_ARRAY(input, Timer_ID);
...
void Animate_Map_Animation(tnecs_In *input) {
    ...
    /* -- Get components arrays -- */
    struct Timer        *timer_arr      = TNECS_C_ARRAY(input, Timer_ID);
    struct MapAnimation *mapanim_arr    = TNECS_C_ARRAY(input, MapAnimation_ID);
...
void Animate_Unit_Move_onMap(tnecs_In *input) {
    ...
    /* -- Get components arrays -- */
    struct Timer                *timer_arr      = TNECS_C_ARRAY(input, Timer_ID);
    struct UnitMoveAnimation    *unitmove_arr   = TNECS_C_ARRAY(input, UnitMoveAnimation_ID);
...
void Animate_Cutscene(tnecs_In *input) {
    ...
    /* -- Get components arrays -- */
    struct Timer    *timer_arr      = TNECS_C_ARRAY(input, Timer_ID);
    struct Cutscene *cutscene_arr   = TNECS_C_ARRAY(input, Cutscene_ID);
...
void Draw_Scene(        tnecs_In *input) {
    ...
    Scene *scene_arr = TNECS_C_ARRAY(input, Scene_ID);
...
void Draw_Map(          tnecs_In *input) {
    ...
    Map *map_arr = TNECS_C_ARRAY(input, Map_ID);
