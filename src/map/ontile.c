...
void _Map_Unit_Put(struct Map *map, u8 col, u8 row, tnecs_E entity) {
    ...
    struct Position *pos = IES_GET_C(map->world, entity, Position);
...
void Map_Unit_Put(struct Map *map, u8 col, u8 row, tnecs_E entity) {
    ...
    /* -- Preliminaries -- */
    // tnecs_E current = map->darrs.unitmap[row * Map_col_len(map) + col];
    _Map_Unit_Put(map, col, row, entity);
    DARR_PUT(map->units.onfield.arr, entity);

    /* -- Adding MapHPBar -- */
    if (!IES_E_HAS_C(map->world, entity, MapHPBar)) {
        TNECS_ADD_C(map->world, entity, MapHPBar_ID);
        struct MapHPBar *map_hp_bar = IES_GET_C(map->world, entity, MapHPBar);
    ...
    /* -- Updating unit pos -- */
    struct Unit     *temp_unit   = IES_GET_C(map->world, entity, Unit);
    struct Sprite   *temp_sprite = IES_GET_C(map->world, entity, Sprite);
...
void Map_Unit_Swap(struct Map *map, u8 old_col, u8 old_row, u8 new_col, u8 new_row) {
    ...
    tnecs_E unit_old = map->darrs.unitmap[old_i];
    tnecs_E unit_new = map->darrs.unitmap[new_i];
...
void Map_Unit_Move(struct Map *map, u8 col, u8 row, u8 new_col, u8 new_row) {
    ...
    tnecs_E entity = map->darrs.unitmap[old_i];
...
tnecs_E *Map_Unit_Gets(struct Map *map, u8 army) {
    tnecs_E *unit_ents = NULL;
    unit_ents = DARR_INIT(unit_ents, tnecs_E, 16);
    tnecs_E current_unit_ent;
    struct Unit *current_unit;
    for (u8 i = 0; i < DARR_NUM(map->units.onfield.arr); i++) {
        current_unit_ent = map->units.onfield.arr[i];
        current_unit = IES_GET_C(map->world, current_unit_ent, Unit);
...
tnecs_E Map_Unit_Get_Boss(struct Map *map, u8 army) {
    ...
    for (int i = 0; i < DARR_NUM(map->units.onfield.arr); i++) {
        ...
        struct Unit *boss = IES_GET_C(map->world, ent, Boss);
...
void Map_Breakable_onBroken(struct Map *map, tnecs_E breakable) {
}

void Map_Door_onOpen(struct Map *map, tnecs_E door) {
}

void Map_Chest_onOpen(struct Map *map, tnecs_E chest) {
}
...
tnecs_E _Map_Unit_Remove_List(struct Map *map,  tnecs_E entity) {
    ...
    if (found > -1) {
        // tnecs_E out = map->units.onfield.friendlies[found];
    ...
    if (found > -1) {
        // tnecs_E out = map->units.onfield.enemies[found];
...
tnecs_E _Map_Unit_Remove_Map(struct Map *map, u8 col, u8 row) {
    SDL_assert(map->darrs.unitmap != NULL);
    tnecs_E out = map->darrs.unitmap[row * Map_col_len(map) + col];
...
void Map_Unit_Remove(struct Map *map, tnecs_E entity) {
    ...
    tnecs_E ontile_ent = map->darrs.unitmap[index];
    SDL_assert(ontile_ent > TNECS_NULL);
    // SDL_Log("%s %d %d", __func__, ontile_ent, map->darrs.unitmap[index]);
    SDL_assert(ontile_ent == entity);
    struct Sprite *sprite = IES_GET_C(map->world, entity, Sprite);
    if (sprite != NULL)
        sprite->visible = false;

    if (IES_E_HAS_C(map->world, entity, MapHPBar))
        TNECS_RM_C(map->world, entity, MapHPBar_ID);
