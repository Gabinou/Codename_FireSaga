...
void Map_Global_Danger_Reset(struct Map *map) {
    memset(map->darrs.global_dangermap, 0,
           sizeof(*map->darrs.global_dangermap) * map->row_len * map->col_len);
...
void Map_Danger_Reset(struct Map *map) {
    memset(map->darrs.dangermap, 0, sizeof(*map->darrs.dangermap) * map->row_len * map->col_len);
    map->flags.shading_changed = true;
}

void Map_Danger_Add(struct Map *map, i32 *danger) {
    map->darrs.dangermap = matrix_plus_noM(map->darrs.dangermap, danger, map->row_len * map->col_len);
    map->flags.shading_changed = true;
}

void Map_Danger_Sub(struct Map *map, i32 *danger) {
    map->darrs.dangermap = matrix_sub_noM(map->darrs.dangermap, danger, map->row_len * map->col_len);
    map->flags.shading_changed = true;
}
...
i32 *Map_Act_To(  struct Map *map, MapAct mapto) {
    ...
    // matrix_print(map->darrs.attacktomap, map->row_len, map->col_len);
...
i32 *Map_Danger_Compute(struct Map *map, tnecs_entity unit_ent) {
    ...
    // memset(map->darrs.temp, 0, sizeof(*map->darrs.temp)*map->row_len * map->col_len);
    // TODO :
    ...
    map->darrs.temp = matrix_plus_noM(map->darrs.temp, map->darrs.attacktomap,
                                      map->row_len * map->col_len);
    // matrix_print(map->darrs.temp, map->row_len, map->col_len);
...
i32 *Map_Costmap_PushPull_Compute(struct Map *map, tnecs_entity unit_ent) {
    ...
    SDL_assert(map->world   != NULL);
    memset(map->darrs.costmap, 0, sizeof(*map->darrs.costmap) * map->col_len * map->row_len);
    struct Unit *unit = IES_GET_COMPONENT(map->world, unit_ent, Unit);
...
void Map_globalRange(struct Map *map, u8 alignment) {
    ...
    memset(map->darrs.global_rangemap, 0,
           sizeof(*map->darrs.global_rangemap) * map->row_len * map->col_len);
