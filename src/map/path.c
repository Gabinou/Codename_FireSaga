...
i32 *Map_Movemap_Compute(struct Map *map, tnecs_E unit_ent) {
    SDL_assert(map      != NULL);
...
i32 *Map_Act_From(struct Map *map, MapAct map_from) {
    ...
    struct Unit *agg_unit       = IES_GET_C(gl_world, map_from.aggressor, Unit);
    /* Get dft position */
    struct Position *agg_pos    = IES_GET_C(gl_world, map_from.aggressor, Position);
    struct Position *dft_pos    = IES_GET_C(gl_world, map_from.defendant, Position);
    ...
    tnecs_E *occupymap = (map_from.move == true) ?
...
i32 *Map_Danger_Compute(struct Map *map, tnecs_E unit_ent) {
    SDL_assert(map          != NULL);
    SDL_assert(gl_world   != NULL);

    Map_Costmap_Movement_Compute(map, unit_ent);
    struct Position *position   = IES_GET_C(gl_world, unit_ent, Position);
    struct Unit *unit           = IES_GET_C(gl_world, unit_ent, Unit);
...
i32 *Map_Costmap_PushPull_Compute(struct Map *map, tnecs_E unit_ent) {
    ...
    for (u8 i = 0; i < Map_row_len(map) * Map_col_len(map); i++) {
        ...
        if (ontile_unit_ent <= TNECS_NULL)
            continue;
        struct Unit *ontile_unit = IES_GET_C(gl_world, ontile_unit_ent, Unit);
        if (ontile_unit != NULL)
...
i32 *_Map_Costmap_Movement_Compute(Map *map, Unit *unit) {
    ...
    for (int i = 0; i < len; ++i) {
        ...
        tnecs_E ontile_unit_ent = map->darrs.unitmap[i];
...
b32 Map_Costmap_Skip(Map *map, tnecs_E unit_ent) {
...
void Map_globalRange(struct Map *map, u8 alignment) {
    ...
    for (int i = 0; i < num_unit_entities; i++) {
        struct Unit     *temp_unit  = IES_GET_C(gl_world, unit_entities[i], Unit);
        struct Position *temp_pos   = IES_GET_C(gl_world, unit_entities[i], Position);
        struct Range range = Range_default;
