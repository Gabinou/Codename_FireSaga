...
struct Point stats_menu_elem_pos[SM_ELEM_NUM] = {
    /* SM_ELEM_SEX */       {SEX_ICON_X_OFFSET, SEX_ICON_Y_OFFSET},
    /* SM_ELEM_TITLE */     {TITLE_X_OFFSET,     TITLE_Y_OFFSET},
    /* SM_ELEM_NAME */      {NAME_X_OFFSET,      NAME_Y_OFFSET},
    /* SM_ELEM_CLASS */     {CLASS_X_OFFSET,     CLASS_Y_OFFSET},
...
void StatsMenu_Draw_Name(struct StatsMenu *stats_menu, SDL_Renderer *renderer) {
    ...
    srcrect.x = stats_menu->unit->sex * SM_SEX_TILESIZE;
    srcrect.y = 0;
    dstrect.w = srcrect.w;
    dstrect.h = srcrect.h;
...
void StatsMenu_Draw_Stats(struct StatsMenu *stats_menu, SDL_Renderer *renderer) {
    ...
    x += STATBAR_COL1_X_OFFSET, y += STATBAR_COL2_Y_OFFSET;
...
void StatsMenu_Draw_Item(struct StatsMenu *stats_menu, SDL_Renderer *renderer, int i) {
    ...
    /* - Item name - */
    int width = PixelFont_Width_Len(stats_menu->pixelnours_big, nstr_toUpper(item_name));
    ...
    if (width <= limit) {
        ...
        y = item_y_offset;
        PixelFont_Write_Len(stats_menu->pixelnours, renderer, nstr_toUpper(item_name), x, y);
        return;
    }
    ...
    char *last_space = strrchr(item_name, ' ');

    if (i == UNIT_HAND_RIGHT) {
        size_t len2 = strlen(last_space + 1);
        size_t len1 = strlen(item_name) - len2 - 1;

        int w1 = PixelFont_Width(stats_menu->pixelnours_big, item_name,  len1);
...
void StatsMenu_Draw(struct MenuComponent *mc, SDL_Texture *rt, SDL_Renderer *renderer) {
    ...
    can_equip     &= stats_menu->unit->equipped[UNIT_HAND_RIGHT] == 0;
    can_equip     &= Unit_canEquip_inHand(stats_menu->unit, UNIT_HAND_LEFT);
    can_equip     &= Unit_canEquip_inHand(stats_menu->unit, UNIT_HAND_RIGHT);
    if (can_equip) {
        stats_menu->update = true;
