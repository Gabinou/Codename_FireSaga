...
void PopUp_Unit_Set(struct PopUp_Unit *pu, struct Game *sota) {
    SDL_assert(pu != NULL);
    struct Unit *unit = IES_GET_COMPONENT(gl_world, sota->hovered.unit_entity, Unit);
...
void _PopUp_Unit_Set(struct PopUp_Unit *pu, struct Unit *unit) {
    ...
    pu->unit   = unit;
    pu->update = true;
    SDL_assert(gl_unit_names[Unit_Order(pu->unit)].data != NULL);
}
...
void PopUp_Unit_Update(struct PopUp_Unit *pu, struct n9Patch *n9patch,
    ...
    /* -- NAME -- */
    struct Point pos;
    const s8 name = gl_unit_names[Unit_Order(pu->unit)];
