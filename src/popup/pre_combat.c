...
void PreCombatPopup_Load(struct PreCombatPopup *pcp, tnecs_entity aggressor,
    ...
    *n9patch                = n9Patch_default;
    ...
    struct Unit *agg_unit    = IES_GET_COMPONENT(gl_world, pcp->aggressor, Unit);
    struct Unit *dft_unit    = IES_GET_COMPONENT(gl_world, pcp->defendant, Unit);
    /* Getting unit positions */
    struct Position *agg_pos = IES_GET_COMPONENT(gl_world, pcp->aggressor, Position);
    struct Position *dft_pos = IES_GET_COMPONENT(gl_world, pcp->defendant, Position);
