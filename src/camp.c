...
void Camp_Job_Hire(struct Camp *in_camp, uint_fast16_t unit_id, uint_fast16_t job_id) {
    ...
    uf8 *jobs_arr = (uf8 *)&in_camp->guards;
    uf8 *job_arr = jobs_arr + ((job_id - 1) * CAMP_JOB_MAX);
...
uf8 Camp_hasJob(struct Camp *in_camp, uint_fast16_t unit_id) {
    SOTA_Log_Func("%d\t%s\t" STRINGIZE(__LINE__), call_stack_depth++, __func__);
    uf8 *jobs_arr = (uf8 *)&in_camp->guards;
    bool found = false;
    uf8 has_job = CAMP_JOB_NULL;
...
uf8 *Camp_Job_Get(struct Camp *in_camp, uint_fast16_t job_id) {
    SOTA_Log_Func("%d\t%s\t" STRINGIZE(__LINE__), call_stack_depth++, __func__);
    SDL_assert(in_camp->workers[job_id] < (max_jobs[job_id] - 1));
    uf8 *jobs_arr = (uf8 *)&in_camp->guards;
    uf8 *job_arr = jobs_arr + (job_id * CAMP_JOB_MAX);
    return (job_arr);
    SOTA_Log_Func("%d\t%s\t" STRINGIZE(__LINE__), --call_stack_depth, __func__);
}
...
void Camp_Jobs_Clear(struct Camp *in_camp) {
    ...
    for (int_fast8_t i = CAMPJOB_GUARD; i < CAMPJOB_END ; ++i) {
        uf8 *job_arr = jobs_arr + ((i - 1) * CAMP_JOB_MAX);
