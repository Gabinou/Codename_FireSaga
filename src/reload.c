...
void Reload_Entities_Archetype(struct Game *sota, entity_reload_f reload_func,
    ...
    size_t num_archetypes = sota->ecs.world->bytype.num_archetype_ids[flag_id];
    for (size_t tsub = 0; tsub < num_archetypes; tsub++) {
        size_t archetype_id = sota->ecs.world->bytype.archetype_id[flag_id][tsub];
...
void Reload_Entities(struct Game *sota, entity_reload_f reload_func, size_t flag_id,
    ...
    // size_t num_entities = sota->ecs.world->bytype.num_entities[flag_id];

    // for (size_t i = 0; i < num_entities; i++) {
    //     tnecs_entity entity = sota->ecs.world->bytype.entities[flag_id][i];
    //     size_t component_id   = TNECS_COMPONENT_TYPE2ID(component);
    //     void *struct_ptr      = tnecs_get_component(sota->ecs.world, entity, component_id);
