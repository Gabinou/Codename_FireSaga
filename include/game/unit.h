...
/* --- Party utilities --- */
void Game_Party_Load(  struct Game *s, if16 *to_load_ids, size_t load_num);
void Game_Party_Clear( struct Game *s);
void Game_Party_Unload(struct Game *s, if16 *to_unload,   size_t unload_num);

tnecs_entity_t Game_Unit_Entity_Create(struct Game *s, if16 in_unit, struct Point in_pos);

/* --- Unitmap --- */
void Game_UnitsonMap_Free(struct Game *s);
void Game_putPConMap(struct Game *s, if16 *u, struct Point *p, size_t l);
